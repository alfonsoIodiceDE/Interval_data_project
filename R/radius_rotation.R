radius_rotation <- function(scaled_C,scaled_R,first_only=F,supp=0){
  ...
  A <- scaled_C/sqrt(n)
  B <- scaled_R/sqrt(n)
  r <- dim(scaled_C)[2] #numero di colonne della matrice dei centri 
  U <- matrix(0,r,r) #si crea una matrice (r x r) con elementi nulli 
  #r1 <- dim(scaled_C)[1] #numero di righe della matrice dei centri
  ...
  #--------------------------------------------------#
  #################################################### 
  u <- svd(t(scaled_R)%*% scaled_R)$u
  
  Rnr <- scaled_R%*%u%*%Ti #proiezioni dei raggi per matrice di rotazione iniziale Tc=K1%*%K2
  
  if(supp>0){
    Rnrsupp <- scaled_Rsupp%*%u%*%Ti
  }
  ...
  # last_rot_rad_coord <- scaled_R %*% u %*% OKT
  last_rot_rad_coord <- scaled_R %*%  OKT %*% u
  ctr <- sqrt(diag(last_rot_rad_coord[,1:2]%*%t(last_rot_rad_coord[,1:2])))/sqrt(diag(data.matrix(scaled_R)%*%t(data.matrix(scaled_R))))
  ...
  out$last_rot_rad_coord <- last_rot_rad_coord
  # out$last_rot_rad_coord2 <- scaled_R %*%  OKT %*% u
  out$counter <- counter
